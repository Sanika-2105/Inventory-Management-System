...
/**
 * Servlet implementation class LogsProcessed
 */
public class LogsProcessed extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int issuer_id=0,prev_quantity=0,id=0;
		String issuer_name=request.getParameter("pname");
		int quantity=Integer.parseInt(request.getParameter("quantity"));
		String date=request.getParameter("date");
	    String time=request.getParameter("time");
		System.out.println(item_id+" "+item_name+" "+issuer_name+" "+quantity+" "+date+" "+time);
		try {
			PreparedStatement pstmt=con.prepareStatement("select * from processed_item where id=? and pname=?");
		    ...
		    ResultSet rs2=pstmt.executeQuery();
		       ...
		       if(rs2.next())
		       {
		    	   ...
		    	   if(quantity>0)
		    	   {
		    		   ...
		    		   {
		    		   PreparedStatement stmt=con.prepareStatement("insert into logs_processed values (?,?,?,?,?,?,?)");
						...
						if(i>0)
						{
							response.sendRedirect("viewLogsProcessed.jsp");
							System.out.println(i);
						} 
						int new_quantity=prev_quantity-quantity; 
					    System.out.println(new_quantity);
					    PreparedStatement pstmt2=con.prepareStatement("update processed_item set quantity=? where id=?");
